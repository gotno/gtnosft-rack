...
struct AdapterInfo {
  // adapter identifier (e.g., "eth0", guid on Windows)
  std::string name;
  // human-readable adapter name
  std::string description;
  // ipv4 address in dotted-decimal notation
  std::string ip_address;
  // ipv4 netmask in dotted-decimal notation
  std::string netmask;
  // whether the adapter is operational and has an IP
  bool is_connected;
  // true if wireless adapter
  bool is_wifi;
  // true if wired Ethernet adapter
  bool is_ethernet;
...
inline bool is_adapter_usable(const IP_ADAPTER_ADDRESSES* adapter) {
  ...
  if (adapter->IfType != IF_TYPE_ETHERNET_CSMACD && adapter->IfType != IF_TYPE_IEEE80211) return false;
  return true;
}
...
inline std::vector<AdapterInfo> get_adapters_unix() {
  ...
  for (struct ifaddrs* ifa = ifap; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    if (sockaddr_to_ip_string(ifa->ifa_addr, adapter.ip_address) && sockaddr_to_ip_string(ifa->ifa_netmask, adapter.netmask)) {
    adapters.push_back(adapter);
    }
  }
